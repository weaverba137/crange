...
/*
 * M_PI should be defined in math.h.  If not, define it here.
 */
#ifndef M_PI
...
{
    if(z.i==0.0){
        return(z.r >= 0 ? 0.0 : M_PI);
    } else if(z.r==0.0) {
        if(z.i==0) return(0.0); else return(z.i > 0.0 ? M_PI_2 : -M_PI_2);
    } else {
 ...
 */
{
    ...
    if(z.r<0){
        result=Csub(Complex(log(M_PI),0.0),Cadd(result,Cln(Csin(RCmul(M_PI,z)))));
    }
    return(result);
}
